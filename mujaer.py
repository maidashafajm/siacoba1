...
def supabase_login(email, password):
    ...
    # ✅ Kalau gagal login, langsung return
    ...
    user_res = requests.get(f"{AUTH_URL}/user", headers=headers_user)
    user_data = user_res.json()

    # ✅ Return format sesuai login code
    return {
                ...
                if not valid:
                    ...
                    try:
                        ...
                        if "id" in auth_response:
                            st.success("✅ Akun berhasil dibuat! Silakan cek email verifikasi.")
                        else:
                            st.error(f"❌ Gagal daftar: {auth_response.get('msg')}")

                        if auth_response.user:
                            # Simpan juga ke database lokal
                            c = conn.cursor()
                            c.execute("""INSERT INTO users 
                                        (email, username, password, role, verified, verification_token)
                                        VALUES (?, ?, ?, ?, ?, ?)""",
                                     (email, username, hash_password(password), role, 0, ''))
                            conn.commit()
                            
                            st.success("✅ Registrasi berhasil! Cek email untuk verifikasi.")
                            st.info("📧 Email verifikasi telah dikirim ke inbox Anda")
                            st.balloons()
                        else:
            ...
            if not email or not password:
                ...
                try:
                    ...
                    if "Email not confirmed" in str(e):
                        st.error("❌ Email belum diverifikasi! Cek inbox Anda.")
                    else:
