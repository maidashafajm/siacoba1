                ...
                if not valid:
                    ...
                    try:
                        ...
                        if auth_response.get("user"):
                            ...
                            c = conn.cursor()
                            c.execute("""
                                INSERT INTO users (email, username, role, verified, verification_token)
                                VALUES (?, ?, ?, ?, ?)
                            """, (email, username, role, 0, ''))
                            conn.commit()
...
def verify_email_page():
    ...
    # Ambil token dari query parameters
    ...
    try:
        ...
        # Untuk verifikasi, kita perlu menggunakan endpoint khusus
        # Supabase biasanya handle verifikasi otomatis, tapi kita bisa cek status
        ...
        if response.status_code == 200:
            ...
            if email:
                ...
                st.info("Anda akan diarahkan ke halaman login dalam 3 detik...")
                import time
                time.sleep(3)
                st.session_state.page = 'login'
                st.rerun()
    ...
    with st.form("forgot_form"):
        ...
        if submit:
            if not email:
                ...
                try:
                    # Gunakan Supabase recovery dengan redirect ke app kita
                    data = {
                        "email": email,
                        "options": {
                            "redirect_to": "https://siacoba1-4aazpzevgfmcxpegbymy24.streamlit.app/"  # Ganti dengan URL app Anda
                        }
                    }
                    ...
                    if res.status_code == 200:
                        ...
                        st.info("📧 Cek inbox atau folder spam Anda")
                    else:
                        st.error("Gagal mengirim email reset. Pastikan email terdaftar.")
                ...
                if not valid:
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            st.error(f"Gagal reset password: {response.text}")
...
def main():
    ...
    query_params = st.query_params
    if 'type' in query_params and query_params['type'][0] == 'recovery' and 'access_token' in query_params:
