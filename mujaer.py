...
def register_page():
    ...
    with st.form("register_form"):
        ...
        submit = st.form_submit_button("Daftar", type="primary")
        ...
        if submit:
            if not email or not password or not username:
                ...
                if not valid:
                    ...
                    try:
                        # Daftar user ke Supabase
                        auth_response = supabase_signup(email, password, username, role)
                        ...
                        if auth_response.get("user"):
                            ...
                            c = conn.cursor()
                            c.execute("""
                                INSERT INTO users (email, username, password, role, verified, verification_token)
                                VALUES (?, ?, ?, ?, ?, ?)
                            """, (email, username, hash_password(password), role, 0, ''))
                            conn.commit()

                            st.success("✅ Akun berhasil dibuat! Periksa email untuk verifikasi.")
                            st.balloons()
                        else:
                            st.error("❌ Gagal daftar. Pastikan email belum digunakan!")
...
def forgot_password_page():
    ...
    with st.form("forgot_form"):
        ...
        submit = st.form_submit_button("Kirim Link Reset", type="primary")
        ...
        if submit:
            if not email:
                ...
                try:
                    res = supabase_forgot_password(email)
                    st.success("✅ Link reset password telah dikirim ke email Anda!")
                    st.info("📧 Cek inbox atau folder spam")
                except Exception as e:
                    st.error(f"Error: {str(e)}")
...
def reset_password_page():
    ...
    # cek query params yang dikirim Supabase: biasanya ada type=recovery & access_token
    qp = st.experimental_get_query_params()  # more robust than st.query_params for fragments?
    access_token = None

    # common keys: 'access_token' OR Supabase sometimes uses 'token' or in 'type=recovery'
    if "access_token" in qp:
        access_token = qp["access_token"][0]
    elif "token" in qp:
        access_token = qp["token"][0]
    elif "access_token" in st.session_state:
        access_token = st.session_state["access_token"]

    if not access_token:
        st.error("Token reset tidak ditemukan di URL. Pastikan kamu membuka link dari email yang dikirim Supabase.")
        st.info("Jika link membuka homepage, baca solusi alternatif di dokumentasi atau hubungi support.")
        return
    ...
    if st.button("Reset Password"):
        if not new_password:
            ...
            try:
                ...
                # PATCH /auth/v1/user dengan Bearer <access_token>
                resp = requests.patch(f"{SUPABASE_URL}/auth/v1/user", json={"password": new_password}, headers=headers)
                if resp.status_code in (200, 204):
                    st.success("Password berhasil diubah. Silakan login.")
                    st.session_state.page = "login"
                else:
                    st.error(f"Gagal reset: {resp.text}")
            except Exception as e:
                st.error(f"Error: {e}")
...
# Main routing
def main():
    # Check query params (gak perlu lagi verify/reset manual)
    ...
    if not st.session_state.logged_in:
        ...
        if st.session_state.role == 'kasir':
            kasir_dashboard()
        elif st.session_state.role == 'akuntan':
            akuntan_dashboard()
