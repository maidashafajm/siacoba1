        ...
        if submit:
            if not email or not password or not username:
                ...
                if not valid:
                    ...
                    try:
                        # ✅ Cek email sudah terdaftar atau belum di SQLite
                        c = conn.cursor()
                        c.execute("SELECT email FROM users WHERE email=?", (email,))
                        existing = c.fetchone()
                        
                        if existing:
                            st.error("❌ Email sudah terdaftar! Silakan gunakan email lain atau login.")
                            return
                        ...
                        # ✅ Cek response
                        if auth_response.get("user") or auth_response.get("id"):  # ← Tambah cek "id"
                            # Simpan user ke SQLite
                            c.execute("""
                                INSERT INTO users (email, username, role, verified, verification_token, password)
                                VALUES (?, ?, ?, ?, ?, ?)
                            """, (email, username, role, 0, '', hash_password(password)))  # ← Tambah password
                            conn.commit()

                            st.success("✅ Akun berhasil dibuat! Periksa email untuk verifikasi.")
                            st.info("📧 Klik link di email untuk aktivasi akun")
                            st.balloons()
                        else:
                            error_msg = auth_response.get("message", "Gagal daftar")
                            if "already registered" in error_msg.lower():
                                st.error("❌ Email sudah terdaftar di Supabase!")
                            else:
    ...
    with st.form("forgot_form"):
        ...
        if submit:
            if not email:
                ...
                try:
                    # ✅ Cek email terdaftar di SQLite dulu
                    c = conn.cursor()
                    c.execute("SELECT email FROM users WHERE email=?", (email,))
                    user = c.fetchone()
                    ...
                    # ✅ Panggil fungsi dengan parameter email
                    res = supabase_forgot_password(email)  # ← INI PENTING ADA PARAMETER EMAIL

                    st.success("✅ Link reset password telah dikirim ke email Anda!")
                    st.info("📧 Cek inbox atau folder spam Anda")
                    st.info("⏰ Link berlaku selama 1 jam")
